...
using CodeDesk.Desktop.NativeHost.WindowHost;
...
namespace CodeDesk.Desktop.CefGlue
{
    ...
    public static unsafe class CefRuntime
    {
        ...
        {
            if (_loaded) return;
            var path = Path.Combine(AppDomain.CurrentDomain.SetupInformation.ApplicationBase, "runtimes", AppBuilder.Is64Bit ? "win-x64" : "win-x86", "libcef.dll");
            if (AppBuilder.Platform == RuntimePlatform.Windows)
            {
                if (AppBuilder.Is64Bit)
                {
                    Win32Api.LoadLibraryEx(path, IntPtr.Zero, Win32Api.LoadLibraryFlags.LOAD_WITH_ALTERED_SEARCH_PATH);
                }
            }
        ...
        public static int ExecuteProcess(CefMainArgs args, CefApp application, IntPtr windowsSandboxInfo)
        {
            ...
            try
            {
                return libcef.execute_process(n_args, null, (void*)windowsSandboxInfo);
            }
            finally
            {
        ...
        public static void Initialize(CefMainArgs args, CefSettings settings, CefApp application)
        {
            ...
            try
            {
                ...
                if (libcef.initialize(n_main_args, n_settings, n_app, IntPtr.Zero) != 0)
                {
                    _initialized = true;
                }
                else
